...
class EvolutionStore:
    def __init__(self, root: str | Path):
        ...
        with self.transaction() as db:
            db.executescript("""
                ...
                CREATE INDEX IF NOT EXISTS records_status
                    ON records(kind, json_extract(payload,'$.status'));
                CREATE INDEX IF NOT EXISTS records_candidate
                    ON records(kind, json_extract(payload,'$.candidate_id'));
                CREATE INDEX IF NOT EXISTS events_entity_sequence
                    ON events(entity_id, sequence);
                CREATE INDEX IF NOT EXISTS records_workspace_source
                    ON records(json_extract(payload,'$.run_id'),json_extract(payload,'$.project_id'),
                               json_extract(payload,'$.source_id')) WHERE kind='workspace_source';
                CREATE INDEX IF NOT EXISTS records_mining_source
                    ON records(json_extract(payload,'$.source_id'),json_extract(payload,'$.status'))
                    WHERE kind='mining_job';
            """)
    ...
    def list(self, kind: str, *, limit: int = 100, offset: int = 0) -> list[dict]:
        ...
        with self.transaction(read_only=True) as db:
            rows = db.execute(
                "SELECT id,payload,version FROM records WHERE kind=? ORDER BY rowid LIMIT ? OFFSET ?",
                (kind, limit, offset),
            ).fetchall()
            return [
    ...
    def read_evidence(self, sha256: str, *, _db=None) -> Any:
        with nullcontext(_db) if _db is not None else self.transaction(read_only=True) as db:
