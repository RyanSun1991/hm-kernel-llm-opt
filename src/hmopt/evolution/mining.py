...
contents. Historical patterns require an explicit code-analysis report through
change_analysis; commit messages never decide eligibility. Human curation and
activation are required before scanning a pattern.
...
class Candidate(_Record):
    ...
    repo_path: _Text
    repo_id: _Id | None = None
    workspace_manifest_sha256: Annotated[str | None, Field(pattern=r"^[0-9a-f]{64}$")] = None
    repo_revision: str
    path: str
...
class _Git:
    ...
    def read(
        self, arguments: list[str], cap: int, *, allowed_exit_codes=(0,)
    ) -> tuple[bytes, bool]:
        ...
        try:
            ...
            if code not in allowed_exit_codes and not truncated:
...
def active_pattern_records(records: list[dict]) -> list[dict]:
    """Select the newest active versions without resurrecting retired predecessors."""
    newest, retired = {}, {}
    for row in records:
        data = row["data"]
        pattern = data["pattern"]
        name, version = pattern["pattern_id"], pattern["version"]
        if data["status"] == "retired":
            retired[name] = max(retired.get(name, 0), version)
        elif data["status"] == "active" and (
            name not in newest or version > newest[name]["data"]["pattern"]["version"]
        ):
            newest[name] = row
    return [
        row
        for name, row in sorted(newest.items())
        if row["data"]["pattern"]["version"] > retired.get(name, 0)
    ]
...
def scan_candidates(
    ...
    *,
    ...
    if not active:
        return []
    pathspecs = [] if paths is None else [":(literal)" + _path(p) for p in paths]
    if paths is not None and (not paths or len(paths) > 64 or len(paths) != len(set(paths))):
        raise ValueError("A scan partition requires 1..64 distinct exact file paths")
    raw, clipped = git.read(
        ["ls-tree", "-r", "-z", "-l", commit, "--", *pathspecs],
        min(16 * 1024 * 1024, (max_files + 1) * 4250),
    )
    rows = raw.split(b"\x00")[:-1]
    incomplete = clipped or len(rows) > max_files
    stats["traversal_incomplete"] = incomplete
    consumed = 0
    ...
    for row in rows[:max_files]:
        ...
        if consumed + size > _MAX_SCAN_BYTES:
            incomplete = True
            stats["traversal_incomplete"] = True
            break
        ...
        for pattern in applicable:
            ...
            unresolved = any(
                value.startswith((_HEURISTIC, _TRUNCATED, "SEMANTIC_REVIEW:"))
            ...
            if unresolved:
                reasons.append(
                    "Unresolved semantic applicability or historical provenance requires workbench review."
                )
            ...
            identity = [git.repo_id, commit, pattern.model_dump(), path, source_hash, line]
            if workspace_manifest_sha256:
                identity.append(workspace_manifest_sha256)
            candidate_id = _digest(identity, "candidate_")
            candidate = Candidate(
