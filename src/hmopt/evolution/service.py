...
class ValidationPolicy(Contract):
    ...
    measurement_profile_sha256: str | None = Field(default=None, pattern=r"^[0-9a-f]{64}$")
...
class Plan(Contract):
    ...
    workspace_manifest_sha256: str | None = Field(default=None, pattern=r"^[0-9a-f]{64}$")
...
class EvolutionService:
    def __init__(
        ...
        *,
        ...
        repo_identities: dict[str, str] | None = None,
        ...
        self.repo_identities = {
            str(Path(p).resolve()): i for p, i in (repo_identities or {}).items()
        }
        self.source_workspace = source_workspace

    def repo_identity(self, repo: str | Path) -> str:
        from .mining import _digest

        path = str(Path(repo).resolve())
        return self.repo_identities.get(path, _digest(path, "repo_"))
        ...
        return digest(
            {
                **(
                    ...
                    else {"repo_path": candidate["repo_path"]}
                ),
                **{
                    key: candidate[key]
                    for key in ("path", "source_sha256", "pattern_id", "pattern_version")
                },
                **(
                    {"workspace_manifest_sha256": candidate["workspace_manifest_sha256"]}
                    if candidate.get("workspace_manifest_sha256")
                    else {}
                ),
            }
        )
    ...
    def ingest_history(self, changes: list[ChangeRecord]) -> dict:
        ...
        from .change_analysis import enqueue_history

        with self.store.transaction() as db:
            for change in changes:
                self.store.put(db, "history", change.source_id, change.model_dump(mode="json"))
                enqueue_history(self.store, db, change)
        return {
            "changes": len(changes),
            "source_ids": [change.source_id for change in changes],
            "draft_patterns": [],
            "analysis_required": True,
            "next_action": "Analyze immutable before/after code in OpenCode before distilling patterns.",
        }
    ...
    def mine(
        ...
        *,
        ...
        changes = mine_git_history(
            ...
            repo_id=self.repo_identity(repo),
        )
        ...
        with self.store.transaction() as db:
            ...
            if data.get("review_required"):
                ...
                if (
                    ...
                    or review.get("subject_digest") != pattern_digest(data)
                    or review.get("actor") == data.get("created_by")
                ):
                    raise GateError("Pattern requires an independent review of its current content")
    ...
    def scan(
        ...
        *,
        ...
        _overlays: dict | None = None,
        _workspace_manifest_sha256: str | None = None,
        _scan_checkpoint: tuple[str, int] | None = None,
    ) -> dict:
        if not 1 <= top_k <= 1000:
            raise ValueError("top_k must be 1..1000")
        if self.source_workspace and _workspace_manifest_sha256 is None:
            from .workspace import freeze_workspace

            manifest = freeze_workspace(self.source_workspace, git_bin=self.git_bin)
            target = self._revision(repo, revision)
            matching = [p for p in manifest["projects"] if p["repo_id"] == self.repo_identity(repo)]
            if len(matching) != 1:
                raise GateError("Scan repository is outside the selected workspace")
            matching[0]["revision"] = target
            with self.store.transaction() as db:
                _workspace_manifest_sha256 = self.store.evidence(db, manifest)
        records = self.store.list("pattern", limit=1000) if _patterns is None else _patterns
        if _patterns is None:
            with self.store.transaction(read_only=True) as db:
                if (
                    db.execute("SELECT count(*) FROM records WHERE kind='pattern'").fetchone()[0]
                    > 1000
                ):
                    raise GateError(
                        "Select/shard the pattern registry before exceeding 1000 versions"
                    )
        active = [
            Pattern.model_validate({**r["data"]["pattern"], "status": "active"})
            for r in active_pattern_records(records)
        ]
        if len(active) > 256:
            raise GateError(
        ...
        found = scan_candidates(
            ...
            git_bin=self.git_bin,
            telemetry=coverage,
            repo_id=self.repo_identities.get(str(Path(repo).resolve())),
            paths=_paths,
        ...
        if overlays is None:
            with self.store.transaction(read_only=True) as db:
                overlays = {
                    ...
                    for row in db.execute("SELECT id,payload FROM records WHERE kind='overlay'")
                }
        adjusted = []
        ...
        with self.store.transaction() as db:
            if _scan_checkpoint is not None:
                scan_id, version = _scan_checkpoint
                scan = self.store.get(db, "scan", scan_id)
                if scan["version"] != version or scan["data"]["status"] != "running":
                    raise ConflictError("Scan checkpoint was superseded before candidate archival")
            for candidate in found:
                ...
                if row:
                    ...
                    if (
                        Path(existing["data"]["candidate"]["repo_path"]).resolve()
                        != Path(candidate.repo_path).resolve()
                    ):
                        raise ConflictError(
                            "Logical candidate already belongs to another checkout; use its archived execution location"
                        )
        ...
        *,
        ...
        with nullcontext(_db) if _db is not None else self.store.transaction() as db:
            ...
            targets = {
                ...
                "record_implementation": "implemented",
                "approve_code": "code_approved",
            }
            if action in targets:
                self._assert_batch_scope(db, candidate_id, targets[action])
            if action == "confirm":
                if stage != "discovered" or actor != candidate["owner"]:
                    raise GateError("Only the assigned owner can confirm a discovered candidate")
                pattern = self.store.get(
                    db, "pattern", f"{candidate['pattern_id']}@{candidate['pattern_version']}"
                )["data"]
                if pattern["status"] != "active":
                    raise GateError("Candidate pattern is no longer active; rescan")
                if pattern.get("requires_assessment"):
                    from .research import pattern_digest

                    assessment = data.get("assessment", {})
                    if (
                        assessment.get("result") != "applicable"
                        or assessment.get("subject_digest") != digest(candidate)
                        or assessment.get("pattern_digest") != pattern_digest(pattern)
                    ):
                        raise GateError(
                            "Confirm requires current, code-grounded applicability assessment"
                        )
                ...
                if (
                    ...
                    or plan.workspace_manifest_sha256 != candidate.get("workspace_manifest_sha256")
                ):
                ...
                self._assert_base(candidate, data.get("execution_context"))
                data.update(
                    stage="plan_approved",
                    plan=plan_data,
                ...
                if data.get("execution_context"):
                    execution_repo = self._execution_repo(candidate, data["execution_context"])
                    if self._revision(execution_repo, "HEAD") != revision:
                        raise GateError(
                            "Implementation must be the isolated execution worktree HEAD"
                        )
            ...
            if action in {"confirm", "reject"}:
                from .catalog import archive_approval

                approval_id = archive_approval(self, db, candidate, request, evidence, row["data"])
                data["approval_id"] = approval_id
                data["owner_decision" if action == "confirm" else "rejection"]["approval_id"] = (
                    approval_id
                )
    ...
    def _assert_batch_scope(self, db, candidate_id: str, target_stage: str) -> None:
        if not db.execute(
            "SELECT 1 FROM records WHERE kind='execution_claim' AND id=?", (candidate_id,)
        ).fetchone():
            return
        from .batch import STAGE_ORDER

        claim = self.store.get(db, "execution_claim", candidate_id)["data"]
        batch = self.store.get(db, "execution_batch", claim["batch_id"])["data"]
        item = next(i for i in batch["items"] if i["candidate_id"] == candidate_id)
        if item["status"] != "running" or STAGE_ORDER.index(target_stage) > STAGE_ORDER.index(
            item["target_stage"]
        ):
            raise GateError("Execution exceeds the active batch scope or has not been claimed")

    def _execution_repo(self, candidate: dict, context: dict | None = None) -> str:
        if not context:
            return candidate["repo_path"]
        path = Path(context["repo_path"])
        expected = self.store.root / "worktrees" / digest(candidate["candidate_id"])
        if (
            path != expected
            or path.resolve() != expected
            or context["candidate_digest"] != digest(candidate)
        ):
            raise GateError("Execution worktree binding changed")
        original = self._git(
            candidate["repo_path"], "rev-parse", "--path-format=absolute", "--git-common-dir"
        ).strip()
        actual = self._git(path, "rev-parse", "--path-format=absolute", "--git-common-dir").strip()
        top = self._git(path, "rev-parse", "--show-toplevel").strip()
        if Path(original).resolve() != Path(actual).resolve() or Path(top).resolve() != path:
            raise GateError("Execution worktree belongs to another repository")
        return str(path)

    def _assert_base(self, candidate: dict, context: dict | None = None) -> None:
        repo = self._execution_repo(candidate, context)
        if self._revision(repo, "HEAD") != candidate["repo_revision"]:
        ...
        if self._git(repo, "status", "--porcelain", "--untracked-files=no").strip():
            raise GateError(
    ...
    def validate(
        ...
        *,
        ...
        with nullcontext(_db) if _db is not None else self.store.transaction() as db:
            ...
            self._assert_batch_scope(db, candidate_id, "validated")
            data = deepcopy(row["data"])
            ...
            if manifest_sha:
                manifest = self.store.read_evidence(manifest_sha, _db=db)
                baseline_projects = {
                    p["repo_id"]: p["revision"]
                    for p in [*manifest["projects"], *manifest.get("dependencies", [])]
                }
                target_id = data["candidate"]["repo_id"]
                if baseline_projects.get(target_id) != data["candidate"]["repo_revision"]:
                    raise GateError("Candidate does not match its frozen workspace manifest")
                feature_projects = {**baseline_projects, target_id: implementation["revision"]}
                if (
                    report.baseline.project_revisions != baseline_projects
                    or report.candidate.project_revisions != feature_projects
                ):
                    raise GateError(
                        "A/B project revisions differ from the frozen workspace; only the reviewed target may change"
                    )
            if (
            ...
            from .reports import report_sources, verify_measurement_profile
    ...
    def _journal(self, db: Any, candidate_id: str, data: dict, signal: str, outcome: str) -> None:
        ...
        skill = {
            "skill_id": skill_id,
            "record_scope": "execution_evidence",
    ...
    def handoff(self, candidate_id: str) -> dict:
        ...
        if stage in {"confirmed", "plan_approved"}:
            self._assert_base(data["candidate"], data.get("execution_context"))
        if data.get("execution_context") and data.get("implementation"):
            repo = self._execution_repo(data["candidate"], data["execution_context"])
            if self._revision(repo, "HEAD") != data["implementation"]["revision"]:
                raise GateError("Isolated worktree HEAD differs from recorded implementation")
        return {
            "schema_version": 1,
            "candidate_id": candidate_id,
            ...
            "repo_path": self._execution_repo(data["candidate"], data.get("execution_context")),
            ...
            "method_sha256": data.get("execution_context", {}).get("method_sha256"),
            "evidence": data,
            "recalled_knowledge": self.recall(data["candidate"]["path"]),
            "knowledge_scope": "Local execution evidence; use native memory_recall for Team Memory",
            "contracts": [
                ...
                "For multi-Git candidates bind the workspace manifest in the plan and every project's revision in both experiment arms; only the reviewed target may differ.",
            ],
        }
