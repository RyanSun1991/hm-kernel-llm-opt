...
_WORKBENCH_ROOT = ".opencode/local/workspaces"


def _workbench_kind(uri: str) -> str | None:
    """Select durable workbench artifacts, never arbitrary logs or session state."""
    parts = PurePosixPath(uri).parts[3:]
    if len(parts) == 2 and parts[1] in {"task.md", "capsule.md", "decisions.md"}:
        return "decision"
    ...
    if set(re.split(r"[-_.]", stem)) & {
        ...
        return None
    steps = {
        "research": ("memory", {"research-note.md"}),
        "plan": ("decision", {"plan.md", "plan.json"}),
        "plan-review": ("review", {"independent-plan-review.json", "plan-review.md"}),
        "implementer": ("decision", {"implementation.json", "implementation-note.md"}),
        "reviewer": ("review", {"code-review.json", "code-review.md"}),
        "validator": ("experiment", {"ab-report.json", "correctness-report.json", "validation.md"}),
    }
    if len(parts) >= 4 and parts[2] in steps:
        kind, filenames = steps[parts[2]]
        # Explicit recipe outputs, optionally below a role's attempt directory.
        return kind if parts[-1] in filenames else None
    for prefix, kind in groups.items():
        if stem == prefix or stem.startswith(prefix + "-") or stem.endswith("-" + prefix):
            return kind
        if len(parts) >= 4 and parts[2] in {prefix, prefix + "s"}:
            return kind
    return None
...
def _workspace_manifest(root: Path, repo_id: str) -> dict:
    ...
    pending.append((root / _WORKBENCH_ROOT, "workbench", False))
    consumed = 0
    entries = 0
    exhausted = False
    while pending:
        ...
        try:
            ...
            for child in sorted(children, key=lambda item: item.name):
                ...
                if path.suffix.lower() not in _SUFFIXES:
                    continue
                selected_kind = _workbench_kind(uri) if kind == "workbench" else kind
...
def import_workspace(
    ...
    *,
    ...
    result.update(
        ...
        workspace=str(root),
        whitelist=[value[0] for value in _SOURCE_DIRS]
        + [
            ".opencode/state/*bad_plans.md",
            _WORKBENCH_ROOT + "/<task>/{task,capsule,decisions}.md",
            _WORKBENCH_ROOT + "/<task>/artifacts/{research,plan,review,validation,experiment}*",
        ],
