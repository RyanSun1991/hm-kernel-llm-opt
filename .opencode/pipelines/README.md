# Pipeline Presets

These are reusable OpenCode pipeline cards.

Use them when you want a short task request to expand into a full staged workflow with:

- the right specialist bias
- the right bootstrap docs
- the right skill packs
- the right validation mode
- the right plan-review, code-review, and tester gates
- the right instruction-count optimization bias
