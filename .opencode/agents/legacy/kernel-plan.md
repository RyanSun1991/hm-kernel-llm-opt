...
2. Read `.opencode/config.yaml` + `.opencode/skills/infra/language-config/SKILL.md` — apply the configured language to every prose section.
...
On any turn where you are emitting ideas (typically turn 1, and again after a batch of ideas has been fully verdicted and the human asks for more), run the funnel from `.opencode/skills/scenario/kernel-opt/optimization-funnel/SKILL.md`:
