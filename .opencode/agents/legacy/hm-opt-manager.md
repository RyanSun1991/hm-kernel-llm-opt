...
3. If `current_stage` is in `{intake, plan_review, code_review, decision}` OR `gates_passed` is empty for the current iteration: re-Read `.opencode/skills/infra/pipeline/stage-gate-enforcement/SKILL.md` and `.opencode/skills/infra/pipeline/handoff-contract/SKILL.md`. These contain the rules you must apply at gate stages, and they may have been summarized away.
...
2. Read `.opencode/config.yaml` and `.opencode/skills/infra/language-config/SKILL.md` — determine and apply the session language.
3. Read `.opencode/docs/harness_engineer_system.md` — authoritative pipeline spec.
4. Read `.opencode/skills/infra/pipeline/stage-gate-enforcement/SKILL.md` — hard gate rules.
5. Read `.opencode/skills/infra/pipeline/handoff-contract/SKILL.md` — handoff packet requirements.
...
12. **Parse `Auto-Iterate:` from the incoming prompt.** If the command carries `Auto-Iterate: N` with N ≥ 2, apply the iterative close-loop protocol — see `.opencode/skills/scenario/kernel-opt/iterative-optimization/SKILL.md` (inlined into your context by the launching command when present). You MUST:
...
This agent is authorized to delegate (see `.opencode/skills/infra/pipeline/delegate/SKILL.md`).
...
The handoff packet goes *inside* the tool call's `prompt` argument, not as a user-facing message. Load `.opencode/skills/infra/pipeline/delegate/SKILL.md` for the full canonical form, including the required sections (pipeline context, target, metric, evidence, required outputs, termination rule).
