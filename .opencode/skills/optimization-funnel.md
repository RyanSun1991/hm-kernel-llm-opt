...
1a. **Scope diversity requirement.** Each of the 5 ideas MUST carry a `scope:` tag — one of:
    ...
    Among the 5 ideas, **at least 3 distinct scope tags** must appear. A funnel that emits 5 `function`-scope ideas is invalid and must be re-run after the researcher has done a structural pass over the target — see `research-discipline.md` step 5 (Structural Audit).
