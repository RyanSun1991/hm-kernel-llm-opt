...
When a primary agent that loads `.opencode/skills/human-interaction-memory/SKILL.md` (e.g. `kernel-research`, `kernel-plan`) runs, memory writes happen at **two** points:

1. **Live, per human turn** — the decision log at `.opencode/memory/human_decisions/<target_slug>.md` and the idea ledger at `.opencode/memory/idea_ledger/<target_slug>.md` get appended to / updated on every human verdict, before the agent ends its turn.  This is what makes sessions resumable after compaction.  See `.opencode/skills/human-interaction-memory/SKILL.md` for the full protocol.
