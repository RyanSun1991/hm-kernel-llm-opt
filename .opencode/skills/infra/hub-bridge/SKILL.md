...
> **Two chains run the pipeline during the workbench transition.** Where this file
> says "the manager" / `hm-opt-manager`, read the active chain's hub: `coordinator`
> on the new workbench chain (default since M4), `hm-opt-manager` (`agents/legacy/`)
> on the legacy fallback chain. The hub touchpoints are identical for both.
...
`.opencode/skills/infra/team-memory/SKILL.md`, not this file.
