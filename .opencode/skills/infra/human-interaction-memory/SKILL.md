...
- `researcher` (workbench role; command `/research`) — iteratively refines `<target>_design.md` with human feedback
- `architect` (workbench role; command `/plan`) — funnels optimization ideas past a human expert, records per-idea verdicts, produces `<target>_plan.md`
- `kernel-research` / `kernel-plan` (agents/legacy/, fallbacks until the live comparison is archived) — the pre-workbench equivalents of the two roles above; same stores, same rules
- `kernel-function-research` (agents/legacy/) — per-function deep-dive; may optionally adopt the same stores

Where a rule below names `kernel-research` or `kernel-plan`, it applies equally to the workbench role that replaced it (researcher / architect).
