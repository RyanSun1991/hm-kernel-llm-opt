...
> in this file use the **legacy cast**; on the new workbench chain (default since
> M4) map: `hm-opt-manager` → `coordinator` · research specialists → `researcher` ·
> `kernel-plan-reviewer` / `kernel-code-reviewer` → `reviewer` · `kernel-code-agent`
> → `implementer` · `kernel-tester-agent` → `validator`. Packet contents and
> verification rules are identical for both casts.
...
9. **recommended next route**: `accept` | `kernel-code-agent` | `kernel-source-research` | `iterate` | `reject` — see `agents/legacy/kernel-tester-agent.md` → "Recommended Next Route" for which failure maps to which agent, and the hub's Feedback Routing Table (`skills/infra/pipeline/recipe-execution/SKILL.md` on the new chain; `agents/legacy/hm-opt-manager.md` on the legacy chain) for the hub-side rules
