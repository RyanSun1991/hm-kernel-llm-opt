...
> **Two chains run this pipeline during the workbench transition.** Stage-owner names
> in this file use the **legacy cast**; when the recipe runs on the new workbench
> chain (the default since M4), map them as: `hm-opt-manager` → `coordinator` ·
> research specialists → `researcher` (+ the routed domain pack) ·
> `kernel-plan-reviewer` / `kernel-code-reviewer` → `reviewer` (plan- / code-review
> brief) · `kernel-code-agent` → `implementer` · `kernel-tester-agent` → `validator`.
> Gates, entry/exit conditions, and back-edges are identical for both casts. See
> `recipe-execution/SKILL.md` for the new-chain operational manual.
...
The manager (`hm-opt-manager`) MUST run the **Per-Turn State Rebuild** protocol — defined in `.opencode/agents/legacy/hm-opt-manager.md` under the section of that exact name — at the START of EVERY turn. Not just when uncertain. Not just after 10 exchanges. Every turn.
