...
The same main service now exposes the Evolution optimization workflow. Its 19
...
For first use, follow the [three-step quickstart](EVOLUTION_QUICKSTART_CN.md):
setup generates one config, doctor checks readiness, and workbench commands start the workflow.
See [Evolution configuration and workflow](EVOLUTION_OPENCODE_RUNBOOK_CN.md#2-接入-opencode)
for discovery profiles, state/workspace/artifact roots, human gates and workbench commands.
...
The main server exposes 24 tools: five kernel-index tools and 19 Evolution tools.
...
Evolution tool names retain the `evolution_` prefix. Prefer the single configuration
generated by `hmopt evolve setup --repo ... --owner ...`: set
`HMOPT_EVOLUTION_CONFIG` to that file on the existing MCP service. Operator CLI
commands select the same file with `--config /absolute/config.json`.
...
For advanced or existing deployments, configure the state directory separately
with `HMOPT_EVOLUTION_ROOT` (default `data/evolution`), Git with
`HMOPT_EVOLUTION_GIT_BIN` (default `git`), and their operator-controlled scope with
`HMOPT_EVOLUTION_WORKSPACE_ROOT`, `HMOPT_EVOLUTION_ARTIFACTS_ROOT` and
`HMOPT_EVOLUTION_DISCOVERY_PROFILES`. The last variable points to a JSON profile
file. Use absolute paths and the same state directory as the operator CLI.
For local OpenCode, merge these values into the existing MCP entry's `environment`;
for remote MCP, set them on the server process and restart it. Tool registration
does not create an Evolution database; initialization is deferred until a tool
needs the store. Profile configuration is frozen at server startup.
