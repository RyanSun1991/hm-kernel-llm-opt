...
6. **本地在线消解的 mem0 依赖策略**（v2.2 新增）：① 完全自研复刻 v0.1.x 论文 prompt；② 用 `mem0ai` OSS 包拿基础设施 + 自带消解 prompt（避开 v3 退化）；③ 评估 mem0 Platform。决策影响 Phase 1 工期。
   > **已决策（Phase 1 PoC，2026-06-09）：选 ①（自研），不引入 `mem0ai` 运行时依赖。** 理由：(a) mem0 v3 OSS 已退化为 ADD-only + hash 去重（§3 提示），UPDATE/DELETE 本就要自己复刻；(b) 本设计的**七路关系分类**（§10.1.0）比 mem0 的 ADD/UPDATE/DELETE/NOOP 四路更细，分类逻辑必须自研；(c) 自研可保持**离线确定性**（CI 可跑、无 LLM gateway 依赖），并内建「temporal/conditional/selector/evidence 绝不 delete」纪律。落地：`src/hmopt/memory/local_curator.py`（确定性启发式分类器 + 可注入 LLM 覆盖歧义项）+ `curator_benchmark.py`（48 例，准确率 1.0 / false-delete 0）。**保留再评估点**：当 `local/memory/` 体量增大、ANN 召回成为瓶颈时，再评估选 ② 借 `mem0ai` 的索引/ANN 基础设施（仅基础设施，分类 prompt 仍自带）——届时 §12.3 的索引后端（faiss/LanceDB）一并定。
