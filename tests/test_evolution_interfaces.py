...
@asynccontextmanager
async def local_session(scenario, extra_args=(), extra_env=None):
    env = dict(os.environ)
    for name in tuple(env):
        if name.startswith(("HMOPT_LLM_", "HMOPT_EVOLUTION_")) or name in {
            "OPENAI_API_KEY",
            "OPENAI_BASE_URL",
        }:
            env.pop(name)
    ...
    # The SDK's default stderr may have been captured inside a CliRunner invocation.
    # Windows subprocess creation needs a real descriptor for the child error log.
...
def test_stdio_tool_registry_preserves_operator_boundaries_and_capture_is_unverified(scenario):
    ...
    async def exercise():
        async with local_session(s) as session:
            ...
            assert "workspace_root" not in schemas["evolution_materialize_workspace"]["properties"]
            assert "artifacts_root" not in schemas["evolution_convert_lmbench"]["properties"]
            tool_error(
                await session.call_tool(
                    "evolution_materialize_workspace", {"dispatch_id": "missing"}
                ),
                "evolution.workspace_root",
            )
            tool_error(
                await session.call_tool(
                    "evolution_convert_lmbench", {"candidate_id": s.candidate_id, "manifest": {}}
                ),
                "storage.artifacts.root_dir",
            )
            plan = s.plan_payload()["plan"]
            normalized = tool_value(
                await session.call_tool("evolution_digest", {"kind": "plan", "payload": plan})
            )
            assert normalized["sha256"] == digest(normalized["contract"])
            assert normalized["subject_digest"] == normalized["sha256"]
            tool_error(
                await session.call_tool("evolution_digest", {"kind": "owner", "payload": {}}),
                "Contract kind",
            )
