...
def test_distillation_is_draft_provenance_not_semantic_claims(repo: Path):
    (repo / "loop.c").write_text("read_value();\n")
    base = commit(repo, "initial")
    (repo / "loop.c").write_text("cached_value();\n")
    commit(repo, "perf: reduce instruction work")
    changes = history(repo, after_revision=base)
    patterns = distill_patterns(changes + changes)
    assert len(patterns) == 1
    pattern = patterns[0]
    assert pattern.status == "draft"
    assert pattern.kind == "optimization"
    assert pattern.matcher.all_of == ["read_value();"]
    assert pattern.source_ids == [changes[0].source_id]
    assert "not evidence" in pattern.diagnosis
    assert pattern.pattern_id == distill_patterns(list(reversed(changes)))[0].pattern_id
    assert (
        distill_patterns(
            [changes[0].model_copy(update={"subject": "style", "message": "formatting"})]
        )
        == []
    )
...
def test_correctness_drafts_and_truncated_provenance_stay_workbench(repo: Path):
    source = ChangeRecord(
        source_id="source-fix",
        repo_id="repo-test",
        revision="a" * 40,
        subject="fix: bounds overflow",
        message="fix bounds",
        paths=["x.c"],
        patch="-read_value();\n+checked_read();\n",
        truncated=True,
    )
    pattern = distill_patterns([source])[0]
    assert pattern.kind == "anti_pattern"
    assert any(item.startswith("TRUNCATED:") for item in pattern.preconditions)
