...
def test_all_profiles_reference_existing_assets():
    ...
    # This pins the agent-name and skill-path migrations (os-opt-manager ->
    # hm-opt-manager -> coordinator; flat skills -> 3-tier layout) so the staged
    # prompt can never silently point at nothing again.
...
def test_load_pipeline_profiles_reads_expected_profile():
    ...
    assert profiles["hyperhold_full"].entry_agent == "coordinator"
    assert profiles["generic_full"].primary_goal == "instruction_count"
    assert profiles["generic_full"].plan_reviewer_agent == "reviewer"
    assert profiles["generic_full"].code_reviewer_agent == "reviewer"
    assert profiles["generic_full"].tester_agent == "validator"
...
def test_initialize_pipeline_session_writes_state_and_prompt(tmp_path: Path):
    ...
    assert state["plan_reviewer_agent"] == "reviewer"
    assert state["code_reviewer_agent"] == "reviewer"
    assert state["tester_agent"] == "validator"
    assert prompt.startswith("@coordinator")
...
def test_build_pipeline_prompt_includes_artifacts():
    ...
    assert "Plan reviewer: reviewer" in prompt
    assert "Code reviewer: reviewer" in prompt
    assert "Tester: validator" in prompt
