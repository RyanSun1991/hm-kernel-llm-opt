...
@pytest.mark.parametrize("field", ["title", "body", "target_slug"])
def test_native_export_cli_rejects_nontext_content_before_writes(tmp_path, field):
    value = {
        "title": "Reviewed fact",
        "body": "A reviewed body.",
        "applies_when": ["Valid lifetime"],
        "invalidated_by": ["Lifetime changes"],
        "target_slug": "example-target",
        field: 123,
    }
    path = write_json(tmp_path / "bad-content.json", value)
    root = tmp_path / "state"
    result = cli(
        root,
        "git",
        "export-native-memory",
        "unused",
        "--content",
        path,
        "--memory-root",
...
def test_discovery_cli_stops_at_review_and_resumes_without_duplicate_work(scenario, tmp_path):
    ...
    assert result["data"]["status"] == "awaiting_analysis"
    from evolution_analysis_helpers import complete_fixture_history

    complete_fixture_history(s.service, s.repo)
    result = cli(
        s.service.store.root,
        s.git_bin,
        "run-discovery",
