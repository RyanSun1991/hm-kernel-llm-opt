...
def test_validation_and_promotion_replays_survive_restart(scenario):
    ...
    params = dict(actor="validator", expected_version=s.row["version"], request_id="validate-once")
...
def test_store_rolls_back_failed_transactions_and_keeps_immutable_history(tmp_path):
    ...
    with pytest.raises(ConflictError, match="Immutable record"):
        with store.transaction() as db:
...
def test_repeated_historical_literal_keeps_both_sources_without_overwriting_history(tmp_path):
    ...
    patterns = service.store.list("pattern")
    assert {"source-one", "source-two"}.issubset(
        {source for row in patterns for source in row["data"]["pattern"]["source_ids"]}
    )
...
def test_reimporting_history_preserves_curated_pattern_and_avoids_duplicates(tmp_path):
    ...
    result = service.ingest_history([change])
    pattern_key = result["draft_patterns"][0]
